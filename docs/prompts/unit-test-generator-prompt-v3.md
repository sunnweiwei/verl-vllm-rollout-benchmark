...
- **Alternative-implementation litmus test.** Before finalizing any test, imagine a correct implementation that supports the requested behavior but uses completely different module paths, class names, helper function names, actor names, and file layout from `/workspace/reference_project`. Your final pytest suite must still pass that implementation. If a test would fail only because those internal names differ, the test is over-specified and must be rewritten through an outer behavior surface.
...
- **Name/layout self-check:** scan your final pytest files for every imported dotted path, class name, helper function name, private attribute, source-file read, and exact implementation symbol. For each one, confirm it is anchored in `/workspace/project` or `/workspace/AGENT_PROMPT.md`, not merely in `/workspace/reference_project`. Rewrite any unanchored reference-only usage through an outer behavior surface before finishing.
...
- no final pytest assertion depends on a reference-only module path, class/helper name, private registry, source text snippet, or implementation file layout unless the naming exception is explicitly satisfied.
