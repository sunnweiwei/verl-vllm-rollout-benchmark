...
# Map Megatron LoRA target modules to HF-style module names.
...
# Modules with stacked parameters that need .base_layer suffix for adapter loading.
STACKED_PARAMS = [
    ".q_proj.weight",
    ".q_proj.bias",
    ".k_proj.weight",
...
def build_peft_config_for_inference(lora_config: dict) -> dict:
    """Build a PEFT config dict from a Megatron LoRA config.

    Args:
        lora_config: Megatron lora configuration dictionary.

    Returns:
        A dictionary compatible with inference backends that consume PEFT configs.
    """
...
__all__ = [
    ...
    "build_peft_config_for_inference",
