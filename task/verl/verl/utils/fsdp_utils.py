...
def collect_lora_params(module: FSDP, layered_summon: bool, base_sync_done: bool) -> OrderedDict:
    """
    Collect LoRA params, or full params if the base model has not been synced.
    Works when isinstance(self.module._fsdp_wrapped_module, PeftModel).
    """
    from peft.utils.save_and_load import get_peft_model_state_dict
    ...
    if fsdp_version(module) > 0:
        if layered_summon:
            if not base_sync_done:
                raise ValueError(
                    "To use layered_summon, you must make sure base-model is preloaded, e.g. let "
...
def replace_lora_wrapper(k, peft_config):
    ...
    names for proper weight loading when base model sync is not done.
...
def merged_lora_context(actor, backup_adapters=False):
    ...
    performs operations that need merged weights, then restores the base model
    weights from backup.
