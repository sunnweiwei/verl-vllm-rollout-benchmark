...
class TorchTitanEngine(BaseEngine):
    ...
    def get_per_tensor_param(self, **kwargs):
        ...
        # Convert TorchTitan key names to HuggingFace key names.
        ...
        # the torchtitan model). Some inference backends need lm_head.weight
        # explicitly, so we add it back as a reference to embed_tokens.weight.
        ...
        # 128 with EP=8). Inference needs all experts, so we gather the missing
        # experts by all-gathering each expert weight across the EP process group.
