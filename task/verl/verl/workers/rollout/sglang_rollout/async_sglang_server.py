...
def _extract_prompt_logprobs_sglang(
    ...
    """Shape SGLang input-logprobs into the shared prompt-logprob contract.
    ...
    consumer in ``teacher_manager.AsyncTeacherLLMServerManager`` can treat
    supported teachers interchangeably.
    SGLang returns input logprobs with length ``S == len(input_ids)`` whose first
    entry has ``logprob=None`` (no predicting context). We skip entry 0 and append
    a trailing dummy row to keep the
    ...
    # Entry 0 has logprob=None (no predicting context); skip it.
    for position in range(1, len(input_token_logprobs)):
        if num_prompt_logprobs == 0:
            ...
            # SGLang returns ranked best-first; preserve that ordering.
    ...
    # Trailing dummy row so total length == len(sequence_ids).
...
class SGLangHttpServer:
    ...
    async def generate(
        ...
        if "max_new_tokens" in sampling_params:
            ...
            # Support OpenAI-compatible 'max_tokens' param.
        ...
        # "prompt_logprobs=K" from the distillation teacher: request
