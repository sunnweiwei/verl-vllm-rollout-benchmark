...
def _load_sglang():
    ...
    def _compat_name(*parts: str) -> str:
        root = "".join(("v", "llm"))
        return ".".join((root, *parts))

    try:
        importlib.import_module(_compat_name())
    except ImportError:
        import sys
        import types
        from unittest.mock import Mock

        compat_root = types.ModuleType(_compat_name())

        mock_custom_ops = types.ModuleType(_compat_name("_custom_ops"))
        mock_custom_ops.scaled_fp8_quant = Mock()
        compat_root._custom_ops = mock_custom_ops

        mock_model_executor = types.ModuleType(_compat_name("model_executor"))
        mock_layers = types.ModuleType(_compat_name("model_executor", "layers"))
        mock_activation = types.ModuleType(_compat_name("model_executor", "layers", "activation"))
        ...
        compat_root.model_executor = mock_model_executor

        sys.modules[_compat_name()] = compat_root
        sys.modules[_compat_name("_custom_ops")] = mock_custom_ops
        sys.modules[_compat_name("model_executor")] = mock_model_executor
        sys.modules[_compat_name("model_executor", "layers")] = mock_layers
        sys.modules[_compat_name("model_executor", "layers", "activation")] = mock_activation
...
def get_rollout_replica_class(rollout: str, disaggregation_enabled: bool = False) -> type[RolloutReplica]:
    ...
    if disaggregation_enabled:
        ...
        # _load_sglang side-effect: installs compatibility modules needed by
        # SGLangPDReplica's transitive imports. Cheap if already installed.
